...
namespace PantryPlanner.Services
{
    public class RecipeStepService : IPantryService
    {
        ...
        {
            return Context.RecipeSteps.Where(r => r.RecipeStepId == recipeStepId)
                                            .Include(i => i.Recipe)
                                            .FirstOrDefault();
        }
        ...
        public List<RecipeStep> GetStepsForRecipe(long recipeId, PantryPlannerUser user)
        {
            ...
            return Context.RecipeSteps.Where(r => r.RecipeId == recipeId).ToList();
        }
        ...
        public RecipeStep AddRecipeStep(RecipeStep newRecipeStep, PantryPlannerUser user)
        {
            ...
            Context.RecipeSteps.Add(newRecipeStep);
            Context.SaveChanges();
        ...
        public async Task UpdateRecipeStepAsync(RecipeStepDto updateDto, PantryPlannerUser userUpdating)
        {
            ...
            RecipeStep stepToUpdate = Context.RecipeSteps
        ...
        public RecipeStep DeleteRecipeStep(long recipeStepId, PantryPlannerUser userDeleting)
        {
            ...
            RecipeStep stepToDelete = Context.RecipeSteps.Where(r => r.RecipeStepId == recipeStepId).FirstOrDefault();

            Context.RecipeSteps.Remove(stepToDelete);
            Context.SaveChanges();
        ...
        private int GetNextSortOrderForRecipeStep(long recipeId)
        {
            ...
            if (Context.RecipeSteps.Any(r => r.RecipeId == recipeId) == false)
            {
                return 1;
            }

            int maxSortOrder = Context.RecipeSteps.Where(r => r.RecipeId == recipeId)
