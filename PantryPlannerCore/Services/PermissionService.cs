...
namespace PantryPlanner.Services
{
    public class PermissionService : IPantryService
    {
        ...
        {
            KitchenList kitchenList = Context.KitchenLists.Find(kitchenListId);
        ...
        {
            Kitchen kitchen = Context.Kitchens.Find(kitchenId);
            return UserHasRightsToKitchen(user, kitchen);
        }

        internal bool UserHasRightsToKitchen(PantryPlannerUser user, Kitchen kitchen)
        {
            if (Context == null || user == null || kitchen == null || Context.KitchenUsers == null)
            {
                return false;
            }

            return Context.KitchenUsers.Any(x => x.KitchenId == kitchen.KitchenId && x.UserId == user.Id && x.HasAcceptedInvite == true);
        }
        ...
        internal bool UserOwnsKitchen(PantryPlannerUser user, long kitchenId)
        {
            ...
            Kitchen kitchen = Context.Kitchens.Find(kitchenId);
            return Context.KitchenUsers.Any(x => x.KitchenId == kitchen.KitchenId && x.UserId == user.Id && x.IsOwner);
        }
        ...
        internal bool UserAddedIngredient(long ingredientId, string userId)
        {
            ...
            return Context.Ingredients.Any(i => i.IngredientId == ingredientId && i.AddedByUserId == userId);
        }
        ...
        internal bool UserHasViewRightsToIngredient(long ingredientId, PantryPlannerUser user)
        {
            ...
            return UserAddedIngredient(ingredientId, user) || Context.Ingredients.Any(i => i.IngredientId == ingredientId && i.IsPublic);
        }
        ...
        internal bool UserAddedRecipe(long recipeId, PantryPlannerUser user)
        {
            ...
            return Context.Recipes.Any(r => r.RecipeId == recipeId && r.CreatedByUserId == user.Id);
        }
        ...
        internal bool UserHasViewRightsToRecipe(long recipeId, PantryPlannerUser user)
        {
            ...
            return UserAddedRecipe(recipeId, user) || Context.Recipes.Any(r => r.RecipeId == recipeId && r.IsPublic.GetValueOrDefault(false));
        }
        ...
        internal bool UserHasViewRightsToRecipeIngredient(long recipeIngredientId, PantryPlannerUser user)
        {
            ...
            RecipeIngredient recipeIngredient = Context.RecipeIngredients.Where(r => r.RecipeIngredientId == recipeIngredientId).FirstOrDefault();
        ...
        internal bool UserHasViewRightsToRecipeStep(long recipeStepId, PantryPlannerUser user)
        {
            ...
            RecipeStep recipeStep = Context.RecipeSteps.Where(r => r.RecipeStepId == recipeStepId).FirstOrDefault();
