...
namespace PantryPlanner.Services
{
    public class KitchenUserService : IPantryService
    {
        ...
        public List<KitchenUser> GetAllUsersForKitchenById(long id, PantryPlannerUser userAccessing)
        {
            ...
            Kitchen kitchen = Context?.Kitchens.Find(id);
        ...
        public List<KitchenUser> GetAcceptedUsersForKitchenById(long id, PantryPlannerUser userAccessing)
        {
            ...
            return Context.KitchenUsers
                    .Where(x => x.KitchenId == id && x.HasAcceptedInvite)
                    .Include(x => x.User)
                    .Include(x => x.Kitchen)
                    .Select(k => k).ToList();
        }
        ...
        public List<KitchenUser> GetUsersThatHaveNotAcceptedInviteByKitchenId(long kitchenId, PantryPlannerUser userAccessing)
        {
            ...
            return Context.KitchenUsers
                    .Where(x => x.KitchenId == kitchen.KitchenId && (!x.HasAcceptedInvite || !x.HasAcceptedInvite))
                    .Include(x => x.User)
                    .Include(x => x.Kitchen)
                    .Select(k => k).ToList();
        }
        ...
        public List<KitchenUser> GetMyInvites(string userId)
        {
            ...
            return Context.KitchenUsers.Where(u => u.UserId == userId && !u.HasAcceptedInvite)
                                      .Include(x => x.User)
                                      .Include(x => x.Kitchen)
                                      .ToList();
        }
        ...
        private bool InviteUserToKitchenByUserId(string userId, long kitchenId, PantryPlannerUser userEditing)
        {
            ...
            Context.KitchenUsers.Add(new KitchenUser
            {
                KitchenId = kitchenId,
                UserId = userId,
        ...
        public bool AcceptInviteToKitchenByKitchenId(long kitchenId, PantryPlannerUser userAccepting)
        {
            ...
            KitchenUser kitchenToAccept = Context.KitchenUsers.Where(k => k.KitchenId == kitchenId && k.UserId == userAccepting.Id && k.HasAcceptedInvite == false).FirstOrDefault();
        ...
        public bool DenyInviteToKitchen(long kitchenId, PantryPlannerUser user)
        {
            ...
            KitchenUser inviteToDeny = user.KitchenUser.Where(k => k.KitchenId == kitchenId && k.UserId == user.Id && k.HasAcceptedInvite == false).FirstOrDefault();
            ...
            Context.KitchenUsers.Remove(inviteToDeny);
        ...
        public KitchenUser DeleteKitchenUserFromKitchen(long kitchenId, string userId, PantryPlannerUser userDeleting)
        {
            ...
            KitchenUser userToDelete = Context.KitchenUsers.Where(k => k.KitchenId == kitchenId && k.UserId == userId).FirstOrDefault();
        ...
        public KitchenUser OwnerDeleteKitchenUserByKitchenUserId(long kitchenUserId, PantryPlannerUser user)
        {
            ...
            var kitchenUser = Context.KitchenUsers.Where(k => k.KitchenUserId == kitchenUserId)
                                                 .Include(k => k.User)
                                                 .FirstOrDefault();
            if (kitchenUser == null)
            {
        ...
        public KitchenUser DeleteMyselfFromKitchenByKitchenUserId(long kitchenUserId, PantryPlannerUser user)
        {
            ...
            var kitchenUser = Context.KitchenUsers.Find(kitchenUserId);
            if (kitchenUser == null)
            {
                throw new KitchenUserNotFoundException();
            }
            ...
            var usersBeforeDelete = Context.KitchenUsers.Where(ku => ku.KitchenId == kitchenUser.KitchenId).ToList();
